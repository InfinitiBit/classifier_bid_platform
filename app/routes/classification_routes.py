...
async def classify_document(request: DocumentClassificationRequest):
    ...
    try:
        ...
        if not request.project_metadata:
            raise HTTPException(status_code=400, detail="Project metadata is required")
        
        # Validate required project metadata fields
        if not request.project_metadata.project_type:
            raise HTTPException(status_code=400, detail="Project type is required in metadata")
        
        if not request.project_metadata.industry:
            raise HTTPException(status_code=400, detail="Industry is required in metadata")
        ...
        # Save initial status with more details
        initial_status = {
            "task_id": request.task_id,
            "project_id": request.project_id,
            "status": "processing",
        ...
        return DocumentClassificationResponse(**error_response)

@router.get("/classification-status/{task_id}", response_model=ClassificationStatusResponse)
async def get_classification_status(task_id: str):
    ...
    try:
        ...
        # Convert the stored result to the response format
        return ClassificationStatusResponse(**result)
    except Exception as e:
        logger.exception(f"Error getting classification status: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/classify-document-enhanced", response_model=DocumentClassificationResponse)
async def classify_document_enhanced(request: EnhancedClassificationRequest):
    ...
    try:
        ...
        # Save initial status with enhanced project info
        initial_status = {
            "task_id": request.taskId,
            "project_id": request.project.id,
            "status": "processing",
        ...
        await LocalStorageManager.save_response(request.taskId, initial_status)
        ...
        error_response = {
            "task_id": request.taskId,
            "project_id": request.project.id if request.project else "unknown",
            "status": "error",
            "message": "Failed to initialize enhanced document classification",
            "error": str(e),
            "timestamp": datetime.now().isoformat()
        }
