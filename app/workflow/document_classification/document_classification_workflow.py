...
# Import models
from app.models import classificationStatusResponse, ClassificationResult, ReportDetailed
...
class DocumentClassificationWorkflow:
    ...
    async def _send_status_update(self, status: str, message: str):
        """
        ...
        """
        status_response = classificationStatusResponse(
            status=status,
            message=message
        )
        ...
        logger.info(f"Status update sent: {status} - {message}")
    ...
    def _build_completion_report(self, is_fast_bypass: bool = False) -> List[Dict[str, str]]:
        ...
        if is_fast_bypass:
            ...
            if not is_valid:
                ...
                summary_decision = "Relevant"  # Fixed typo from 'relevent' to 'Relevant'
            else:
    ...
    async def process_classification(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._send_status_update(
                "processing",
                "Document classification workflow started"
            )
            ...
            try:
                ...
                if not content_result["extraction_success"]:
                    ...
                    await self._send_status_update(
                        "failed",
                        f"Content extraction failed: {content_result.get('error')}"
                    )
                ...
                await self._send_status_update(
                    "processing",
                    f"Content extracted successfully: {len(document_content)} characters"
                )
                ...
                await self._send_status_update(
                    "failed",
                    "Content extraction encountered an error"
                )
            ...
            try:
                ...
                await self._send_status_update(
                    "processing",
                    "Checking document size for fast bypass"
                )
                ...
                if file_size < 1000 or len(document_content.strip()) < 50:
                    ...
                    await self._send_status_update(
                        "completed",
                        "Document classified via fast bypass: insufficient content"
                    )
                ...
                await self._send_status_update(
                    "processing",
                    "Fast bypass skipped - proceeding to full analysis"
                )
            ...
            try:
                ...
                await self._send_status_update(
                    "processing",
                    "AI agents analyzing document relevance"
                )
                ...
                if classification_result.get("status") != "completed":
                    ...
                    await self._send_status_update(
                        "failed",
                        "AI agent classification failed"
                    )
                ...
                await self._send_status_update(
                    "processing",
                    f"Document classified as {relevance_status}"
                )
                ...
                await self._send_status_update(
                    "failed",
                    "Agent classification encountered an error"
                )
            ...
            try:
                ...
                await self._send_status_update(
                    "failed",
                    "Failed to format final results"
                )
    ...
    async def _handle_workflow_error(self, error_msg: str, stage: str) -> Dict[str, Any]:
        ...
        # Send error status update first
        await self._send_status_update(
            "failed",
            f"Workflow failed at {stage}: {error_msg}"
        )

        # Build error completion report - backend needs this as finishing signal
        error_completion_report = [
            {
                "attributeName": "summaryDecision",
                "attributeFriendlyName": "Summary Decision",
            ...
            },
            {
                "attributeName": "decisionDetails",
                "attributeFriendlyName": "Decision Details",
                "attributeValue": f"Workflow failed at {stage}: {error_msg}"
            },
            {
                "attributeName": "relevancyPercentage",
                "attributeFriendlyName": "Relevancy Percentage",
        ...
        # Create error result with completion report
        error_result = ClassificationResult(
            isValid=False,
            classificationReport=error_completion_report
        )

        # Format for backend
        formatted_error = error_result.dict()
        ...
        if self.backend_url and self.task_id:
            try:
                await LocalStorageManager.save_response(self.task_id, formatted_error)

                # Send completion notification (backend needs this as finishing signal)
                await notify_backend_completion(self.backend_url, self.task_id, formatted_error)

                logger.info(f"Sent error completion report for task: {self.task_id}")
